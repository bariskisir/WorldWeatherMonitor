...
let WORLD_CITIES: { [key: string]: CountryData } = {};
let locationsLoaded = false;

let cachedAllLocations: Location[] | null = null;
let spatialHash: { [key: string]: Location[] } | null = null;
const GRID_SIZE = 10;
        ...
        if (state.districts) {
          state.districts.forEach((district: any) => {
            ...
            if (district.subdistricts) {
              district.subdistricts.forEach((sub: any) => {
                const subObj: Location = {
                  name: sub.name,
                  lat: sub.lat,
                  lng: sub.lng,
                  country: countryName,
...
function getCitiesInBounds(
  ...
  const maxX = Math.floor(east / GRID_SIZE);
  const minY = Math.floor(south / GRID_SIZE);
  const maxY = Math.floor(north / GRID_SIZE);
...
function findNearestCountry(lat: number, lng: number): string | null {
